...
export default function OutlineCustomInput() {
  ...
  return (
    <div
      className={`bg-light bg-opacity-65 rounded-2xl shadow-lg w-full ${
        selectorStyle.input
    ...
    >
      ...
      <div className="flex-grow">
        <input
          className={`bg-none bg-pastel bg-opacity-65 shadow-sm w-full h-auto rounded-full px-1 py-0.5 text-center transition-opacity duration-300 ease-out ${
            isDisabled ? "opacity-50" : "opacity-100"
          }`}
          disabled={isDisabled}
          value={storedValue}
